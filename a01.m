function [T, ByCombo, Overall] = a01()
% DVHOP_FAST_FINAL_9000_V3_GITHUB
...
%% ============================== SPEED ====================================
USE_PARALLEL = true;
NUM_WORKERS  = 4;     % GitHub ubuntu-latest standard runner: use 4 local workers.
WRITE_XLSX   = false; % Keep false for fastest run; CSV + MAT are always written.

% ---------------- GitHub/distributed sharding ----------------
% Defaults preserve the original behavior when run locally.
% GitHub Actions sets these environment variables for each matrix job.
NumShards = str2double(getenv('RUN_PARTS'));
ShardID   = str2double(getenv('RUN_PART'));
if ~isfinite(NumShards) || NumShards < 1
    NumShards = 1;
end
NumShards = floor(NumShards);
if ~isfinite(ShardID)
    ShardID = 0;
end
ShardID = floor(ShardID);
assert(ShardID >= 0 && ShardID < NumShards, ...
    'RUN_PART must be in the range 0..RUN_PARTS-1.');

runStamp = datestr(now,'yyyymmdd_HHMMSS');
if NumShards > 1
    outDir = fullfile(pwd, sprintf('dvhop_fast_9000_v3_shard_%02d_of_%02d_%s', ...
        ShardID, NumShards, runStamp));
else
    outDir = fullfile(pwd, ['dvhop_fast_9000_v3_' runStamp]);
end
...
FullTotalRuns = nCombos * TrialsPerCombo;
fprintf('DV-Hop FINAL FAST RUN V3 - GITHUB SHARDED\n');
fprintf('  Combos           : %d\n', nCombos);
...
% Build the exact same global 9000 (ComboID, Trial) pairs as V2, then keep
% only the deterministic subset assigned to this shard. No trial is changed.
runCombo = repelem((1:nCombos).', TrialsPerCombo);
runTrial = repmat((1:TrialsPerCombo).', nCombos, 1);
allRunIndex = (1:FullTotalRuns).';
keepThisShard = mod(allRunIndex - 1, NumShards) == ShardID;
runCombo = runCombo(keepThisShard);
runTrial = runTrial(keepThisShard);
TotalRuns = numel(runCombo);

fprintf('  Shard            : %d/%d (zero-based ID=%d)\n', ShardID+1, NumShards, ShardID);
fprintf('  Trials this shard: %d\n', TotalRuns);
...
Overall.ShardID = ShardID;
Overall.NumShards = NumShards;
Overall.FullExperimentTrials = FullTotalRuns;
Overall.ShardTrials = TotalRuns;
